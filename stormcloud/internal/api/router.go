...
func Router(db *sql.DB, hub realtime.HubIface, syncService *ingest.SyncService, scheduler *jobs.Scheduler) http.Handler {
	...
	r.Route(apiBasePath, func(r chi.Router) {
		...
		r.Post("/managed-events/add-match", ManagedEventAddManualMatch(db))
		...
		r.Get("/exports/match-scouting-dummy.csv", MatchScoutingDummyExportCSV(db))
		...
		r.Post("/auth/profile", LocalAuthProfileUpdate(db))
		r.Post("/auth/password", LocalAuthPasswordUpdate(db))
		r.Get("/leaderboard", LocalLeaderboard(db))
		r.Get("/app-settings", AppSettingsGetWithDB(db))
		r.Post("/app-settings", AppSettingsSet(db, syncService))
		r.Get("/users", LocalUsersList(db))
		r.Post("/users/role", LocalUserRoleUpdate(db))
		r.Get("/scouting-schedule", ScoutingScheduleList(db))
		r.Get("/scouting-schedule/me", ScoutingScheduleMe(db))
		r.Post("/scouting-schedule", ScoutingScheduleSave(db))
